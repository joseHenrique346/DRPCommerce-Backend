...
public class Employee : BaseEntity, ITenantEntity
{
    ...
    private Employee(long enterpriseId, string fullName, EmployeeEmail email, string passwordHash, Role roleId, Department departmentId, bool isActive, DateTime hiredAt)
    {
    ...
    #region Functions
    public static Employee Create(long enterpriseId, string fullName, EmployeeEmail email, string passwordHash, Role roleId, Department departmentId, bool isActive, DateTime hiredAt)
    {
        ...
        BaseValidate.ValidateNotNull(roleId, nameof(RoleId));
        BaseValidate.ValidateNotNull(departmentId, nameof(DepartmentId));
    ...
    public void UpdatePersonalInfo(string fullName, EmployeeEmail email, Role roleId, Department departmentId)
    {
        BaseValidate.ValidateNotNullOrEmpty(fullName, nameof(FullName));
        BaseValidate.ValidateMaxLength(fullName, 255, nameof(FullName));
        BaseValidate.ValidateNotNull(email, nameof(Email));
